...
class FollowingTest < ActionDispatch::IntegrationTest
  ...
  test 'フォロー数カウント' do
    post relationships_path, xhr: true, params: { followed_id: @other.id, user_page_id: @other.id }
    get user_path(@user)
    assert_select 'span[class=?]', 'link__num link__num--following', text: @user.following.count.to_s
    assert_select 'span[class=?]', 'link__num link__num--followers', text: @user.followers.count.to_s
    relationship = @user.active_relationships.find_by(followed_id: @other.id)
    delete relationship_path(relationship), xhr: true, params: { user_page_id: @user.id }
    get followers_user_path(@user)
    assert_select 'span[class=?]', 'link__num link__num--following', text: @user.following.count.to_s
